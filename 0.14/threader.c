...
void tb_thread_ticker( struct toobit_space* in_u, unsigned int ticks ){
  ...
  while(ticks--){
	...
	while(ne--){
	  ...
	  if(ob_sz==0){//bin inject
	    space_ptr=(*in_u).space;
	    buf_now-=4;
	    unsigned int loop_c=TB_SERVER_INJECTION_BUFFER_ENTRY_SIZE-1;
	    while(--loop_c){
	      if(*buf_now==42)*space_ptr=42;
	      buf_now++;
	      space_ptr++;
	      }	  
	    }else if(ob_sz<6){//5x5 inject
