...
void *tb_server( void* in_p ){
  ...
  char client_request[TB_SERVER_MAX_REQUEST_LEN];
  ...
  while(1){//---- Start Server Loop
    ...
    while(read_tries--){
      usleep(TB_SERVER_READ_WAIT_US);
      ret=recv(client_sock, client_request+read_size, (TB_SERVER_MAX_REQUEST_LEN-read_size), MSG_DONTWAIT);
      if(ret>0)read_size+=ret;
      }
	  ...
	  if(read_size>5){
		...
		break;
		  ...
		  break;
	        ...
	        case TB_SERVER_COMMAND_GET_INJECT_BINARY://----- Request to binary inject into space
		  client_request[5]=0;
	  	  tb_server_inject_binary(in_u, client_sock, read_size-5, client_request+5);
		  break;
	      #endif
...
#if TB_SERVER_INJECTION == 1
  //-----------------------------------------------
  // 			tb_server_inject_binary()
  // inject data into space 
  //    format 8bit 0 val, then pure universe data
  //-----------------------------------------------
  void tb_server_inject_binary( struct toobit_space *in_u , int socket, int rlen, char *rptr){
    ...
    if(rlen>TB_SERVER_INJECTION_BUFFER_ENTRY_SIZE)rlen=TB_SERVER_INJECTION_BUFFER_ENTRY_SIZE;
    memcpy(target_ptr,rptr,rlen);
    memset(target_ptr+rlen,0,TB_SERVER_INJECTION_BUFFER_ENTRY_SIZE-rlen);
