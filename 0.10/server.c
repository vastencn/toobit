...
char tb_html_template_space[TB_SERVER_HTML_MEMORY_VALUE];
	...
	case TB_SERVER_COMMAND_GET:
	  if(read_size>5){
		...
		break;
	      ...
	      default: tb_server_print_space_one_byte(in_u, client_sock);
	      }
	    }else tb_server_print_space_one_byte(in_u, client_sock);
	  break;
